...
|----------|-------------|----------------|
| `CapitalGains-Setup-x.y.z.exe` (NSIS) | Yes — background poll/download + **silent** install on quit or “Restart now” | Yes |
| `CapitalGains-x.y.z-Portable.exe` | No — manual download only | Yes |
| `CapitalGains-x.y.z-*.dmg` (macOS) | Manual for now (same release channel) | Yes |
